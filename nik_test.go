...
func testNIK(t *testing.T, context spec.G, it spec.S) {
	...
	it("contributes JDK without NIK", func() {
		...
		n, err := jvmvendors.NewNIK(dep, nil, dc, cl, "", nil)
		Expect(err).NotTo(HaveOccurred())

		Expect(n.LayerContributor.ExpectedMetadata.(map[string]any)["cert-dir"]).To(HaveLen(4))
	...
	it("contributes native image to JDK", func() {
		...
		n, err := jvmvendors.NewNIK(jdkDep, niDep, dc, cl, "bin/gu", []string{"install", "--local-file"})
		Expect(err).NotTo(HaveOccurred())
	...
	it("updates after Java 9 certificates", func() {
		...
		j, err := jvmvendors.NewNIK(dep, nil, dc, cl, "", nil)
		Expect(err).NotTo(HaveOccurred())
