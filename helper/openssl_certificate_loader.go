...
type OpenSSLCertificateLoader struct {
	CertificateLoader jvmvendors.CertificateLoader
	Logger            log.Logger
}
...
func (o OpenSSLCertificateLoader) Execute() (map[string]string, error) {
	...
	if !trustStoreWriteable {
		...
		if err != nil {
			...
			o.Logger.Body("If you need the certificates, please provide a writable temporary directory, e.g. by mounting an empty ephemeral volume. Otherwise you can ignore this message.")
			return nil, nil
		}
