...
func testCountClasses(t *testing.T, context spec.G, it spec.S) {
	...
	it("counts files on filesystem", func() {
		Expect(os.WriteFile(filepath.Join(path, "alpha.class"), []byte{}, 0600)).To(Succeed())
		Expect(os.MkdirAll(filepath.Join(path, "bravo"), 0755)).To(Succeed())
		Expect(os.WriteFile(filepath.Join(path, "bravo", "charlie.class"), []byte{}, 0600)).To(Succeed())
	...
	it("skips empty zip/jar files with none in the name", func() {
		Expect(os.WriteFile(filepath.Join(path, "test-none.jar"), []byte{}, 0600)).To(Succeed())
	...
	it("skips bad jar files", func() {
		Expect(os.WriteFile(filepath.Join(path, "bad-jar.jar"), []byte{}, 0600)).To(Succeed())
