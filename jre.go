...
func NewJRE(applicationPath string, dependency libpak.BuildModuleDependency, cache libpak.DependencyCache, distributionType DistributionType, certificateLoader CertificateLoader, metadata map[string]any) (JRE, error) {
...
func ConfigureJRE(configCtx ConfigJREContext) error {
	...
	if configCtx.IsLaunch {
		...
		switch {
		case IsBeforeJava9(configCtx.JavaVersion) && configCtx.DistType == JDKType:
			securityFile = filepath.Join(configCtx.JavaHome, "jre", "lib", "security", "java.security")
		case IsBeforeJava9(configCtx.JavaVersion) && configCtx.DistType == JREType:
